...
Install the Python CLI package with `uv`, `pipx`, or `pip`, then let `sfao` copy managed templates into the project:
...
`sfao` is autonomous and uses package templates through Python `importlib.resources`. It does not depend on symlinks or current-working-directory-relative source paths.
